...
def _emit_results(r, settings: Settings, env: dict, project_id: str, backlog_item_id: str, work_context: dict) -> None:
    total_minutes, total_hours, _ = compute_time_metrics(work_context)
    friction = compute_friction(work_context)
    costs = compute_costs(total_hours, work_context)
    scenario = compute_scenario(total_hours, costs, friction)

    deliverable = {
        "type": "with_agent_scenario",
        "project_id": project_id,
        "backlog_item_id": backlog_item_id,
        "timestamp": now_iso(),
        ...
        "content": {
            ...
            "avoidable_percent": friction["avoidable_percent"],
            **scenario,
        },
    }
    dlv_env = envelope(
    ...
    completed_env = envelope(
        ...
        payload={
            ...
            "evidence": {
                "agent": AGENT_NAME,
                "recovered_hours": scenario["recovered_hours"],
                "recovered_monthly_cost": scenario["recovered_monthly_cost"],
            },
        },
        source=AGENT_NAME,
        correlation_id=env.get("correlation_id") or str(uuid.uuid4()),
...
def _process_message(r, reg, settings: Settings, msg_id: str, fields: dict) -> None:
    ...
    try:
        missing = []
        if work_context.get("hourly_rate") is None:
            missing.append("hourly_rate")
        if not (work_context.get("rows") or []):
            missing.append("rows")

        if missing:
            _emit_clarification(r, settings, env, project_id, backlog_item_id, missing)
            return

        try:
            _emit_started(r, settings, env, project_id, backlog_item_id)
            _emit_results(r, settings, env, project_id, backlog_item_id, work_context)
        except Exception as e:
