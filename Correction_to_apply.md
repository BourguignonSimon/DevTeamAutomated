...
- [x] **Issue:** Orchestrator DLQ entries can miss event metadata when `_dlq` is called with decoded events or missing `event` fields.
  ...
  - **Status:** Done - Enhanced `_dlq` to accept optional `original_event` parameter and preserve event metadata

- [x] **Issue:** Orchestrator Docker image no longer contains demo/test assets after the Dockerfile removed COPY steps.
  ...
  - **Owner:** _(Name or team)_
  - **Status:** Done - Added COPY steps for demo/, tests/, and pytest.ini

- [x] **Issue:** Documentation states `assert_transition` raises `ValueError`, but the implementation raises `IllegalTransition`.
  ...
  - **Owner:** _(Name or team)_
  - **Status:** Done - Updated documentation to correctly state `IllegalTransition` exception
